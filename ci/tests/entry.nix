...
# refused inside it (`called without required argument 'prelude'`), and that is the direction this
...
in
{
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 8;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs genDispatch [ "dispatch" ]);
    expected = false;
  };
  ...
  # scores a selector through the gen-select adapter, so the adapter's own construction over the
  # injected prelude has to be the real one.
  flake.tests.entry.test-the-shims-library-is-live = {
    expr = standalone.adapters.select.selectorSpecificity genSelect.star;
    expected = 0;
  };
