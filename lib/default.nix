...
# mkRule/dispatch/NACs/conflict-resolution/restrict·override·chain/adapters.select, plus the
# declared-stratum vocabulary (groupOf/producesOf/deriveGroup) that lets a rule DECLARE its
# stratum as data instead of firing to observe it. It owns RULE EVALUATION ONLY. The convergence LOOP belongs to gen-resolve (gen-scope.circular's
...
in
{
  ...
  inherit (declared) groupOf producesOf deriveGroup;
