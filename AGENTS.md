...
| Trap | Evidence |
|---|---|
| `mkRule` and `dispatch` are **closed** attrset patterns — a stray key is a hard error, and these errors are **not** catchable by `tryEval` | `error: function 'mkRule' called with unexpected argument 'bogus'` at `lib/core/rule.nix:6:5`; `error: function 'dispatch' called with unexpected argument 'bogus'`; omitting a required key gives `error: function 'mkRule' called without required argument 'produce'` / `… 'dispatch' called without required argument 'groupOrder'`. Positive control: `builtins.attrNames (mkRule { condition = {}; produce = _: _: []; })` ⇒ `["condition","group","identity","nac","overrides","priority","produce","produces"]` |
| `fromFunction` on an **intensional record** yields a usable `identity` but a condition that **throws when forced** — the rule cannot be dispatched | with `i = { name = "iname"; closure = {}; fn = { host, ... }: []; __functor = self: self.fn; }`: `(fromFunction i).identity` ⇒ `"iname"`, but `(fromFunction i).condition` ⇒ `error: 'functionArgs' requires a function`, and dispatching `[ (fromFunction i) ]` raises the same error. Positive control, plain lambda: `(fromFunction ({ host, ... }: [])).condition` ⇒ `{"host":false}`. `ci/tests/rule.nix` `test-from-function-intensional` forces only `r.identity`, so laziness hides this |
...
- **Forgy (1982), *RETE*** — rule = condition (LHS) + action production (RHS); `mkRule`'s `condition`/`produce` pair.
...
- **Palmer et al. (2024), *Intensional Functions*** — rule identity via a four-predicate `isIntensional` check (`isAttrs` + `name`/`__functor`/`closure`) in `lib/core/rule.nix`. Partially unreachable in practice: see the `fromFunction` trap above.

**Informed by** (README's own label; no result claimed): Hedin & Magnusson (2003) *JastAdd*, for open action types with framework-owned dispatch; Batory (2005) *AHEAD*, for the `restrict`/`override`/`chain` combinators; Berry & Boudol (1990) *Chemical Abstract Machine*, for rules-as-reactions.
